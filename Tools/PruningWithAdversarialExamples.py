...
def model_training(model, device, train_dataloader, optimizer, train_acc, train_losses,lambda_l1, epsilon, mean, std):
...
def iterative_pruning(model, device, TrainLoader, TestLoader, TestCLoader, PruningRate, SaveTrainStepsName,ReadMe, epsilon, mean, std, lambda_l1=0, lambda_l2=0.0005, NumEpochsEachPruneRetrain=1,MaximumPruneLevel=0.01, SaveCorruptedAtP=SaveCorruptedAtP): 
  ...
  for i in range(TotalNumPruningIterations):
    ...
    for j in range(NumEpochsEachPruneRetrain):
        model_training(model, device, TrainLoader, optimizer, train_acc, train_losses, lambda_l1, epsilon, mean, std)
